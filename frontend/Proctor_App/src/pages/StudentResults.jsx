  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        const response = await resultsAPI.getStudentResults(studentId);
            ...
            <div className="space-y-4">
              {results.map((result) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-dark-900">{result.test_title}</h4>
                      <div className="flex items-center gap-3">
                        <span className={`text-2xl font-bold ${color}`}>
                          {grade}
                        </span>
      ...
      {selectedResult && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              <div>
                <h4 className="font-semibold text-dark-900 mb-2">{selectedResult.test_title}</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-dark-600">Score:</span>
                    <span className="ml-2 font-semibold">
                ...
                <div className="p-4 bg-yellow-50 border-2 border-yellow-200 rounded-lg">
                  ...
                  <div className="space-y-2">
                    <div className="text-sm">
                      <span className="text-dark-600">Total Violations:</span>
                      <span className="ml-2 font-semibold text-dark-900">
                        {selectedResult.violations.length}
                      </span>
                    </div>
                    <div className="text-sm">
                      <span className="text-dark-600">Risk Score:</span>
                      <span className="ml-2 font-semibold text-dark-900">
                        {selectedResult.risk_score}%
                      </span>
                    </div>
                    <div className="mt-3">
                      <p className="text-xs text-dark-600 mb-2">Violations detected:</p>
                      <ul className="space-y-1">
                        {selectedResult.violations.map((violation, idx) => (
                          <li key={idx} className="text-xs text-yellow-800 flex items-start gap-2">
                            <span>•</span>
                            <span>{violation.type}: {violation.severity} severity</span>
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              {(!selectedResult.violations || selectedResult.violations.length === 0) && (
                <div className="p-4 bg-green-50 border-2 border-green-200 rounded-lg">
                  <p className="text-sm text-green-800">
                    ✓ No violations detected during this test. Great job!
                  </p>
                </div>
              )}
