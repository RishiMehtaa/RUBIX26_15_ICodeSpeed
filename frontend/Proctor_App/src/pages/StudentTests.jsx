...
const StudentTests = () => {
  ...
  useEffect(() => {
    const fetchTests = async () => {
      ...
      // Filter tests based on dates   
      const filteredByDate = allTests.filter(test => {
        if (test.status === 'completed') {
          return true; // Always show completed tests
        }
        
        if (test.status === 'available') {
          const startDate = new Date(test.start_date);
          const endDate = new Date(test.end_date);
          // Show only if current time is between start and end date
          return  endDate > now;
        }
                ...
                <div className="flex-1">
                  ...
                  {test.start_date && test.end_date && (
                    <div className="flex items-center gap-2 text-sm text-dark-600 mb-2">
                      <Calendar className="w-4 h-4" />
                      <span>
                        Available: {new Date(test.start_date).toLocaleDateString()} - {new Date(test.end_date).toLocaleDateString()}
                      </span>
                    </div>
                  )}
                  ...
                  {test.status === 'completed' && test.score !== undefined && (
                    <div className="mt-3 p-3 bg-green-50 rounded-lg inline-block">
                      <div className="text-sm text-green-800 font-medium">
                        Your Score: {test.score}/{test.total_marks} ({((test.score / test.total_marks) * 100).toFixed(1)}%)
                      </div>
                    </div>
                  )}
                </div>
