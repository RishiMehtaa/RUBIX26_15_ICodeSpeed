...
const AdminResults = () => {
  ...
  const calculateStats = () => {
    ...
    const percentages = results.map(r => (r.score / r.total_marks) * 100);
  ...
  // Score distribution data for chart
  const scoreDistribution = [
    { range: '0-20', count: results.filter(r => (r.score / r.total_marks * 100) < 20).length },
    { range: '20-40', count: results.filter(r => {
      const p = (r.score / r.total_marks * 100);
      return p >= 20 && p < 40;
    }).length },
    { range: '40-60', count: results.filter(r => {
      const p = (r.score / r.total_marks * 100);
      return p >= 40 && p < 60;
    }).length },
    { range: '60-80', count: results.filter(r => {
      const p = (r.score / r.total_marks * 100);
      return p >= 60 && p < 80;
    }).length },
    { range: '80-100', count: results.filter(r => (r.score / r.total_marks * 100) >= 80).length },
  ];

  const handleExport = () => {
    const csv = [
      ['Student ID', 'Test Title', 'Score', 'Total Marks', 'Percentage', 'Violations', 'Risk Score', 'Submitted At'].join(','),
      ...filteredResults.map(r => {
        const percentage = ((r.score / r.total_marks) * 100).toFixed(1);
        return [
          r.student_id,
          r.test_title,
          r.score,
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card p-6">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by student ID or test title..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="input-field pl-12 w-full"
          />
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-dark-50 border-b-2 border-dark-100">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-900">Student ID</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-900">Test</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-900">Score</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-900">Violations</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-900">Risk Score</th>
              ...
              {filteredResults.length === 0 ? (
                ...
                filteredResults.map((result) => {
                  ...
                  return (
                    <tr key={result.id} className="hover:bg-dark-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="font-medium text-dark-900">{result.student_id}</div>
                      </td>
                      <td className="px-6 py-4 text-dark-900">{result.test_title}</td>
                      <td className="px-6 py-4">
                        <div>
                          <div className="font-semibold text-dark-900">
                            {result.score}/{result.total_marks}
                          </div>
                          <div className="text-sm text-dark-600">{percentage}%</div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`badge ${violationsCount > 2 ? 'badge-danger' : violationsCount > 0 ? 'badge-warning' : 'badge-success'}`}>
                          {violationsCount}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-dark-200 rounded-full h-2 w-16">
                            <div
                              className={`h-2 rounded-full ${
                                result.risk_score > 50 ? 'bg-red-600' : result.risk_score > 25 ? 'bg-yellow-600' : 'bg-green-600'
                              }`}
                              style={{ width: `${result.risk_score}%` }}
                            />
                          </div>
                          <span className="text-sm text-dark-600">{result.risk_score}%</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-dark-600">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h4 className="text-sm font-medium text-dark-600 mb-1">Student ID</h4>
                  <p className="font-semibold text-dark-900">{selectedResult.student_id}</p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-dark-600 mb-1">Test</h4>
                  <p className="font-semibold text-dark-900">{selectedResult.test_title}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 p-4 bg-dark-50 rounded-lg">
                ...
                <div>
                  <div className="text-sm text-dark-600 mb-1">Percentage</div>
                  <div className="text-2xl font-bold text-dark-900">
                    {((selectedResult.score / selectedResult.total_marks) * 100).toFixed(1)}%
                  </div>
                </div>
                <div>
                  <div className="text-sm text-dark-600 mb-1">Grade</div>
                  <div className="text-2xl font-bold text-dark-900">
                    {(() => {
                      const percentage = (selectedResult.score / selectedResult.total_marks) * 100;
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-4 border-2 border-dark-100 rounded-lg">
                    <div className="text-sm text-dark-600 mb-1">Risk Score</div>
                    <div className="flex items-center gap-2">
                      <div className="text-xl font-bold text-dark-900">{selectedResult.risk_score}%</div>
                      <span className={`badge ${
                        selectedResult.risk_score > 50 ? 'badge-danger' : 
                        selectedResult.risk_score > 25 ? 'badge-warning' : 
                        'badge-success'
                      }`}>
                        {selectedResult.risk_score > 50 ? 'High' : 
                         selectedResult.risk_score > 25 ? 'Medium' : 'Low'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {selectedResult.violations && selectedResult.violations.length > 0 && (
                  <div className="mt-4 p-4 bg-yellow-50 border-2 border-yellow-200 rounded-lg">
                    <h5 className="font-semibold text-yellow-900 mb-2">Detected Violations:</h5>
                    <ul className="space-y-1">
                      {selectedResult.violations.map((violation, idx) => (
                        <li key={idx} className="text-sm text-yellow-800 flex items-start gap-2">
                          <span>•</span>
                          <span>
                            {violation.type} - {violation.severity} severity
                            {violation.timestamp && ` at ${new Date(violation.timestamp).toLocaleTimeString()}`}
                          </span>
                        </li>
                      ))}
                    </ul>
              ...
              <div className="p-4 bg-dark-50 rounded-lg">
                <h4 className="text-sm font-medium text-dark-600 mb-2">Submission Details</h4>
                <p className="text-sm text-dark-900">
                  Submitted on {selectedResult.submitted_at ? new Date(selectedResult.submitted_at).toLocaleString() : 'N/A'}
                </p>
                <p className="text-sm text-dark-900 mt-1">
                  Duration: {selectedResult.duration} minutes
                </p>
              </div>
            </div>
          </div>
